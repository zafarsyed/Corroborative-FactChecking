...
public class SparqlQueryGenerator {
	...
	public void GeneratorSparqlQueries(Statement input, int path_Length) throws ParseException
	{
		...
		else if (path_Length==2)
		{
			...
			// Generate and collect queries by extending the generic query with necessary where condition, respectively
			sparqlQueries.put(INPUTSUBJECT_AS_SUBJECT.toString().replace("@", "")+";"+INPUTOBJECT_AS_OBJECT.toString().replace("@", ""), path_Length);
			sparqlQueries.put(INPUTSUBJECT_AS_SUBJECT.toString().replace("@", "")+";"+INPUTOBJECT_AS_SUBJECT.toString().replace("@", ""), path_Length);
			sparqlQueries.put(INPUTSUBJECT_AS_OBJECT.toString().replace("@", "")+";"+INPUTOBJECT_AS_OBJECT.toString().replace("@", ""), path_Length);
			sparqlQueries.put(INPUTSUBJECT_AS_OBJECT.toString().replace("@", "")+";"+INPUTOBJECT_AS_SUBJECT.toString().replace("@", ""), path_Length);
